...
- **问题**: 需要一个专门的页面展示所有标签，方便用户浏览和选择
- **解决方案**: 
  - 创建了 `/tags` 路径的页面，展示所有已使用的标签
  - 实现分页功能，每页显示50个标签
  - 显示每个标签关联的文章数量
  - 使用与其他列表页一致的设计风格
  - 技术实现包括服务器组件、客户端组件和API路由的协同工作

### 侧边栏UI优化

- **问题**: 侧边栏中的"查看更多标签"和"更多文章"链接占用空间且不够简洁
- **解决方案**: 
  - 移除底部的蓝色文字链接
  - 在标题栏右侧添加灰色箭头图标，点击可跳转到相应列表页
  - 确保箭头与标题文字的中心线对齐
  - 移除标题下方的灰色下划线，使设计更加简洁
  - 使用精确的CSS定位确保视觉对齐

### 首页文章数量调整

- **问题**: 首页显示的最新文章数量需要调整以展示更多内容
- **解决方案**: 
  - 将首页最新文章数量从10篇增加到12篇
  - 修改API请求参数，确保获取正确数量的文章
  - 客户端组件自动适应新的文章数量

### 文章列表分页优化

- **问题**: 所有文章页面的分页规则需要优化，以提供更好的浏览体验
- **解决方案**: 
  - 将每页显示的文章数量从25篇调整为20篇
  - 同时修改前端请求参数和后端默认值，确保一致性
  - 优化分页导航，提供更直观的用户体验
...
# 方法1：直接启动开发服务器（默认端口3000）
npm run dev

# 方法2：使用自动化启动脚本（端口3099）
./start-dev.sh
```

### 使用自动化启动脚本

项目提供了一个自动化启动脚本 `start-dev.sh`，它可以：

1. 自动检查端口 3099 是否被占用，如果被占用则自动终止占用进程
2. 清理旧的构建文件（删除 .next 目录）
3. 重新构建项目
4. 在端口 3099 上启动服务器
...
启动后，在浏览器中打开 [http://localhost:3099](http://localhost:3099) 查看结果。
