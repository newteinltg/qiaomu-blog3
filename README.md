...
- **功能增强**：在HTML全网页模式下的浮动信息条中添加管理员编辑按钮
- **用户体验改进**：管理员登录状态下可直接从HTML页面跳转到后台编辑界面
- **技术实现**：
  - 修改了`FloatingInfoBar`组件，添加了编辑图标和链接
  - 更新了`HtmlPageLayout`组件，支持传递文章ID参数
  - 使用`useSession`钩子检测管理员登录状态，仅对管理员显示编辑按钮
- **安全性**：编辑按钮仅在用户登录状态下显示，确保只有管理员可以编辑内容
